...
//uint32_t theMinecraftPlayer = 0x014CF2E4;
//uint32_t MultiPlayerLocalPlayer = 0x0140A360;
/*int wcLenOfs = 0x00CB32D8;
int wcslen(const wchar_t * wchar) {
	return Call<int>(wcLenOfs, wchar);
}*/

/*int isFireFunc = 0x001B32C0;
int isExplosionFunc = 0x001B2590;
int isProjectileFunc = 0x001B2588;*/
...
struct DataFixerUpper { //need to reverse


};

struct ItemEntity { //need to reverse


};

struct Entity { //need to reverse
...
struct LivingEntity { //need to reverse


};

struct AxisAlignedBB { //need to reverse


};

struct ModifiableAttributeMap {
...
struct MultiplayerLocalPlayer { //your local player class used for online and offline, other localplayer class is useless at all times
...
/*uint32_t GetInstance_MinecraftEntity()
{
	return theMinecraftPlayer;
}*/
struct TheMinecraft {
...
struct Menu {
	...
	//void AddSubOption(const wchar_t * opt, EMenus::eMenus subMenu);
...
/*namespace Enchantments {
	uint32_t PROTECTION, FIRE_PROTECTION, FEATHER_FALLING, BLAST_PROTECTION, PROJECTILE_PROTECTION, RESPIRATION, AQUA_AFFINITY, THORNS, DEPTH_STRIDER, FROST_WALKER, BINDING_CURSE, SHARPNESS, SMITE, BANE_OF_ARTHROPODS, KNOCKBACK, FIRE_ASPECT, LOOTING, EFFICIENCY, SILK_TOUCH, UNBREAKING, FORTUNE, POWER, PUNCH, FLAME, _INFINITY, LUCK_OF_THE_SEA, LURE, MENDING, VANISHING_CURSE, IMPALING, RIPTIDE, LOYALTY, CHANNELING;
	void RegisterEnchantments() {
		PROTECTION = *(uint32_t*)(0x014C8F44);
		FIRE_PROTECTION = *(uint32_t*)(0x014C8F48);
		FEATHER_FALLING = *(uint32_t*)(0x014C8F4C);
		BLAST_PROTECTION = *(uint32_t*)(0x014C8F50);
		PROJECTILE_PROTECTION = *(uint32_t*)(0x014C8F54);
		RESPIRATION = *(uint32_t*)(0x014C8F58);
		AQUA_AFFINITY = *(uint32_t*)(0x014C8F5C);
		THORNS = *(uint32_t*)(0x014C8F60);
		DEPTH_STRIDER = *(uint32_t*)(0x014C8F64);
		FROST_WALKER = *(uint32_t*)(0x014C8F68);
		BINDING_CURSE = *(uint32_t*)(0x014C8F6C);
		SHARPNESS = *(uint32_t*)(0x014C8F70);
		SMITE = *(uint32_t*)(0x014C8F74);
		BANE_OF_ARTHROPODS = *(uint32_t*)(0x014C8F78);
		KNOCKBACK = *(uint32_t*)(0x014C8F7C);
		FIRE_ASPECT = *(uint32_t*)(0x014C8F80);
		LOOTING = *(uint32_t*)(0x014C8F84);
		EFFICIENCY = *(uint32_t*)(0x014C8F88);
		SILK_TOUCH = *(uint32_t*)(0x014C8F8C);
		UNBREAKING = *(uint32_t*)(0x014C8F90);
		FORTUNE = *(uint32_t*)(0x014C8F94);
		POWER = *(uint32_t*)(0x014C8F98);
		PUNCH = *(uint32_t*)(0x014C8F9C);
		FLAME = *(uint32_t*)(0x014C8FA0);
		_INFINITY = *(uint32_t*)(0x014C8FA4);
		LUCK_OF_THE_SEA = *(uint32_t*)(0x014C8FA8);
		LURE = *(uint32_t*)(0x014C8FAC);
		MENDING = *(uint32_t*)(0x014C8FB0);
		VANISHING_CURSE = *(uint32_t*)(0x014C8FB4);
		IMPALING = *(uint32_t*)(0x014C8FB8);
		RIPTIDE = *(uint32_t*)(0x014C8FBC);
		LOYALTY = *(uint32_t*)(0x014C8FC0);
		CHANNELING = *(uint32_t*)(0x014C8FC4);
	}
	char* getEnchantmentEffect(uint32_t effect) {
		if (effect == PROTECTION) return "PROTECTION";
		if (effect == FIRE_PROTECTION) return "FIRE_PROTECTION";
		if (effect == FEATHER_FALLING) return "FEATHER_FALLING";
		if (effect == BLAST_PROTECTION) return "BLAST_PROTECTION";
		if (effect == PROJECTILE_PROTECTION) return "PROJECTILE_PROTECTION";
		if (effect == RESPIRATION) return "RESPIRATION";
		if (effect == AQUA_AFFINITY) return "AQUA_AFFINITY";
		if (effect == THORNS) return "THORNS";
		if (effect == DEPTH_STRIDER) return "DEPTH_STRIDER";
		if (effect == FROST_WALKER) return "FROST_WALKER";
		if (effect == BINDING_CURSE) return "BINDING_CURSE";
		if (effect == SHARPNESS) return "SHARPNESS";
		if (effect == SMITE) return "SMITE";
		if (effect == BANE_OF_ARTHROPODS) return "BANE_OF_ARTHROPODS";
		if (effect == KNOCKBACK) return "KNOCKBACK";
		if (effect == FIRE_ASPECT) return "FIRE_ASPECT";
		if (effect == LOOTING) return "LOOTING";
		if (effect == EFFICIENCY) return "EFFICIENCY";
		if (effect == SILK_TOUCH) return "SILK_TOUCH";
		if (effect == UNBREAKING) return "UNBREAKING";
		if (effect == FORTUNE) return "FORTUNE";
		if (effect == POWER) return "POWER";
		if (effect == PUNCH) return "PUNCH";
		if (effect == FLAME) return "FLAME";
		if (effect == _INFINITY) return "INFINITY";
		if (effect == LUCK_OF_THE_SEA) return "LUCK_OF_THE_SEA";
		if (effect == LURE) return "LURE";
		if (effect == MENDING) return "MENDING";
		if (effect == VANISHING_CURSE) return "VANISHINH_CURSE";
		if (effect == IMPALING) return "IMPALING";
		if (effect == RIPTIDE) return "RIPTIDE";
		if (effect == LOYALTY) return "LOYALTY";
		if (effect == CHANNELING) return "CHANNELING";
		return "NO ENCHANTMENT FOUND";
	}
};*/
...
/*enum EnchantmentType{
	ARMOR = 0x0151DFD8,
	ARMOR_FEET = 0x0151DFDC,
};*/

/*namespace EnchantmentProtection {
	enum Type {
		ALL = 0x01551364,
		FIRE = 0x01551368,
		FALL = 0x0155136C,
		EXPLOSION = 0x01551370,
		PROJECTILE = 0x01551374,
	};
}*/
...
/*
struct MobEffects {
int SPEED = 0x014C9B24;
int SLOWNESS = 0x014C9B28;

...
void gameRender_Hook(uint32_t r3, uint32_t r4) 
{
	...
	if (FirstInfoMessage)
	{
		if (InWorld())
		{
			Color BLACK_OPACITY{ 15, 15, 15, 220 };
			NewDrawRect(-100, -100, 2000, 2000, BLACK_OPACITY);

			if (FirstInfoMessage1)
			{
				...
				*(int*)0x3092CD81 = 0x00000000; //TEXT CREATIVE
				...
				Dialog::Show(" DownCraft SPRX \n\nFor enable the SPRX you need to use the RTM and press the button 'Enable SPRX'\nFor any problems contact the discord server");
			}

			FirstInfoMessage1 = false;

			if (Buttons::IsMCButtonPressed(Buttons::X))
			{
				FirstInfoMessage = false;
			}
		}
	}
...
{	
	if (devmenu)
	{
		...
		Dialog::ProgressBar("DownCraft SPRX Developer Version\nOpen menu press  + \nMenu made by Misakiii !", "Loading...", 20);
		Dialog::End();
	}
	else
	{
		Dialog::msgdialog_mode = Dialog::MODE_RUNNING;
		Dialog::ProgressBar("DownCraft SPRX Public Version\nOpen menu press  + \nMenu made by Misakiii !", "Loading...", 20);
		Dialog::End();
	}

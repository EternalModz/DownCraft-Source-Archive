...
{
    // <Credits>
    // Theme credits go to CCAPI Manager on GitHub and also to EternalModz for modifying it for DownCraft's GUI.
...
public class cTabControl : TabControl
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        using (Pen Right = new Pen(Color.FromArgb(25, 25, 25)))
        {
        ...
        for (int T = 0; T <= TabPages.Count - 1; T++)
        {
            ...
            if (SelectedIndex == T)
            {
                using (SolidBrush Selection = new SolidBrush(Color.FromArgb(25, 25, 25)))
                {
                ...
                using (Pen Line = new Pen(Color.FromArgb(25, 25, 25)))
                {
